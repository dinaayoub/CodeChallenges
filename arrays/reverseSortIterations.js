...
function reverseSort(arr) {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    let j = i + min(arr.slice(i, arr.length));
    // console.log(i, j);
    if (i < j) {
      arr = reverse(arr, i, j);
    } else if (j < i) {
...
function min(arr) {
  ...
  // console.log('minimum = ', minimum, 'and index = ', index);
  return index;
}

function reverse(arr, start, end) {
  // console.log('before reverse', arr, 'start', start, 'end', end);
...
console.log(reverseSort([4, 2, 1, 3]));
console.log(reverseSort([1, 2]));
console.log(reverseSort([7, 6, 5, 4, 3, 2, 1]));